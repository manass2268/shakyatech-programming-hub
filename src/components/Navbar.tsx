"use client"; // Yeh zaroori hai kyunki hum useState use kar rahe hain
import React, { useState } from 'react';
import Image from 'next/image';

export default function Navbar() {
  // Mobile menu ko open/close track karne ke liye state
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  return (
    <div className="w-full font-sans shadow-md relative">
      {/* Top Bar - Abhi ke liye hide kar diya gaya hai */}
      <div className="hidden bg-[#081839] text-gray-300 text-[11px] py-2">
        <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
          ...
          <div className="flex gap-5 items-center">
            ...
            <span className="flex items-center gap-1">✉️ info@shakyatech.com</span>
          </div>
        </div>
      </div>
      ...
      <header className="bg-white sticky top-0 z-50 w-full overflow-hidden">
        <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 lg:py-4 flex justify-between items-center">
          ...
          {/* Desktop Links Section (Laptop/PC ke liye) */}
          <nav className="hidden lg:flex items-center justify-center space-x-5 xl:space-x-8 text-[14px] xl:text-[15px] font-bold text-[#081839] flex-shrink-0">
            <a href="#" className="text-blue-600 border-b-2 border-blue-600 pb-1 whitespace-nowrap">Home</a>
            <a href="#" className="hover:text-blue-600 transition-colors whitespace-nowrap">About Us</a>
          ...
          {/* Button & Mobile Menu Icon Section */}
          <div className="flex items-center justify-end gap-3 flex-shrink-0 flex-1">
            ...
            >
              {/* Agar menu open hai toh (X) dikhaye, nahi toh (☰) */}
              {isMobileMenuOpen ? '✕' : '☰'}
            </button>
          </div>
        </div>
      ...
      {/* Mobile Menu Dropdown (Sirf tab dikhega jab isMobileMenuOpen true hoga) */}
      {isMobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full bg-white border-t border-gray-200 shadow-xl z-40 flex flex-col">
          <nav className="flex flex-col px-6 py-4 space-y-4 text-sm font-bold text-[#081839]">
            <a href="#" className="text-blue-600 pb-2 border-b border-gray-100">Home</a>
            <a href="#" className="hover:text-blue-600 pb-2 border-b border-gray-100">About Us</a>
            ...
            {/* Apply Now button mobile menu mein bhi de diya hai taaki easily access ho sake */}
